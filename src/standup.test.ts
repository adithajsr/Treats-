...
//Passes
  ...
  describe('standup/active/v1 test', () => {
    beforeEach(() => {
      ...
      badUser = requestAuthRegister('valsaascidemail@gmail.coma', '123aazcazxzbc!@#', 'Jane', 'Does');
  ...
  describe('standup/send/v1 test', () => {
    ...
    test('successful standup send multiple', async () => {
      ...
      expect(testRequest).toStrictEqual({});
      const testRequest2 = requestStandupSend(badUser.bodyObj.token, testChannel.bodyObj.channelId, 'janedoe successful standup send');
      ...
      expect(checkSent.bodyObj.messages[0].message).toStrictEqual(
        ...
        'janedoe: janedoe successful standup send'
      );
    });
  });
